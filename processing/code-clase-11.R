...
---
# title: "Práctica 11 EMV"
# author: "Grupo 5 y Grupo 7 EMV 2020"
# date: "20-08-2020"
---
  
## El objetivo de esta practica es revisar un apartado de transformacion de variables y otro analisis 
## de supuestos y robustez estadistica de regresion. Utilizaremos la base de datos ELSOC 2018 para analizar los determinantes de la participación cuidadana
...
# ELSOC cuenta con una muestra final de 3748 casos en el 2018
...
# ---- 4. Medición y transformación de variables ---- 

# ---- 4.1. Creacion Indice ---- 

## Para aunar preguntas referentes a la participacon politica, se usaran cuatro donde se les pregunta por su frecuencia de participacion
## en determinados evenetos. Se emplean escalas likert de 5 categorias para medir dica participacion 

plot_stackfrq(elsoc[,c("part01","part02","part03","part04")]) + theme(legend.position="bottom")

## Como sabemos, para crear un indie primero debemos verificar si nuetsros indicadores estan correlacionados

corrplot.mixed(cor(select(elsoc,part01,part02,part03,part04),
                   use = "complete.obs"))

## La matriz nos muestra correlaciones moderadas entre los indicadores = 0.25 la mas baja y 0.5 la mas alta.
## Una vez hecho esto se decide crear un indice sumatorio simple. Para ello se usa la funcion mutate y rowSums()

elsoc <- elsoc %>% mutate(partpol=rowSums(select(., part01,part02,part03,part04))) #con mutate se crea una nueva variable y con rowSums se suman los indicadores 

descr(elsoc$partpol,style = "rmarkdown",stats = "common", transpose = T,headings = F)

plot_frq(data = elsoc$partpol,type = "hist",show.mean = T) # hacemos un plot para observar su distribucion

## Entonces: 
# Los valores del indice van desde 4 hasta 20 
# La media es 5.47 y la mediana es 4 
# Esto da cuenta de la proporcion importante que respondio "nunca" [categoria 4] en los indicadores, por ell son represnetados con alta frecuencia en este histograma

# ---- 4.2. Recueprar casos perdidos o Missing's ---- 
...
elsoc$inghogar_i <- set_label(elsoc$inghogar_i,"Ingreso total del hogar (imputada)") # nueva variable
...
sjmisc::descr(elsoc[,c("inghogar","inghogar_i","tamhogar","ing_pcap")],
              show =c("label", "n", "NA.prc", "mean", "md","sd")) %>% knitr::kable(digits = 2) #descriptivo

## Finalmente, vemos que la variable [tamhogar] posee un 0.19% de NA's. Por ende
## la nueva variable [ing_pcap] aumenta un poco sus NA's a un 5.23%


## ---- 4.3. Ingresos como variable categorica ---- 

# Teniendo el ingreso per capita podemos calcular categorias de ingresos como quintiles o deciles. 
# Es decir, como una variable categorica. 
...
## Por ultimo, si queremos recuperar ese 5.23% (n=196) de NA's podemos generar una categoria adicional
## para los datos perdidos, vale decir, recodiricamos los NA's para que se incluyan como nueva categoria
...
## Así, recuperando con esta nueva categoria podemos usar estos datos para posteriores analisis que veremos a continuacion empleando la variable ingresos
...
## En términos generales, el sentido y significación estadística de los coeficientes del Modelo 4 se mantiene respecto al Modelo 3.
## Ademas, si observamos que el modelo sin casos influyentes presenta una mejora en ajuste. Por lo tanto, los análisis posteriores se realizaran en base a este modelo


# ---- 6.2. Linealidad ---- 

## Analisis de la distribucion de los residuos respecto a la recta de regresion 
## Los residuos deben ser independientes de los valores predichos y la presencia de un patron no lineal es señal de que el modelo esta espcificando incorrectamente
ggplot(fit04, aes(.fitted, .resid)) +
  geom_point() +
  geom_hline(yintercept = 0) +
  geom_smooth(se = TRUE)

## Podemos ver un patron de distribucionde los resdio. Para mejorar la estimacion haremos una transofrmacion de variables. Veamos edad e ingresos
...
## Interpretacion: 
# El beta de log(ingreso) es de 0.24 lo cual quiere decir que por una unidad de procentaje (1%) que se incrementen los ingresos per 
# capita del hogar, el promedio del indice de participacion politica auemnta en un 0.24 (o bien un 0.0024) manteniendo todas las variables constantes.
# EL beta es signifiativo a un 99%. 

## Comparando modelos 5 (ingresos como quintiles) y 6 (ingresos logaritmo) podemos ver que es mejor quedarse con el 5 dada la cantidad de N 

# ---- 6.3. Homocedasticidad ---- 
car::ncvTest(fit05)
lmtest::bptest(fit05)

## Tanto el test de Breush-Pagan como el de Cook-Weisberg indican que hay problemas de heterocedasticidad de los resdius
## puesto que el p-value > 0.05. Por tanto se rechaza la H0 = varianza del error es constante

## En vista de este problema, debemos calcular los errores robustos para nuestra ultima estimación para corregir los problemas de heterocedasticidad y estimar el ultimo modelo
...
## Los resultdos del modelo con ES robusto indican que las estimaciones son robustas a la presencia de heterocedasticidad en los reisiduos, 
## debido a que la signifcancia del beta se mantiene respecto al modelo 5. En suma, la heterocedasticidad no es problematica

# ---- 6.4. Multicolinealidad ---- 
car::vif(fit04)
car::vif(fit05)

## Vemos que en el modelo que no incorpora edad cuadratica no hay probelmas de multicolinealidad. Pero al incorporar la edad cuadratica el VIF es de 6.2, por lo que es problematico ese modelo. 
